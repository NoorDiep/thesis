...
def get_data(lag):
    ...
    df_drivers = pd.DataFrame([df_drivers_diff['Sent']]).T
...
def getForecast(x, y, n_train, n_tv, n_test, h, diff_lag):
    ...
    return resultsAR, resultsARX, pd.DataFrame(error).T, pd.DataFrame(errorX).T
...
########## CSSED plots ##########
CSSED_AR5 = getCSSED(error5[6], error5_diff[6])
SSEAR1 = getCSSED(error1, errorX1)
getCSSEDplot(SSEAR1, h=1,dates=date_test)